...
class Runner(object):
    def __init__(self, args, env, handles, models, play_handle, model_dir=None,
                 render_dir=None,  writter=None):
        self.env = env
        self.models = models
        self.max_steps = args.max_steps
        self.handles = handles
        self.render_every = args.render_time if args.render else 0
        self.render = args.render
        self.save_every = args.save_every
        self.print_every = args.print_every
        self.tau = args.tau
        self.train = args.train_mode
        self.cuda = args.cuda
    ...
    def run(self, variant_eps, iteration, n_round, win_cnt=None):
        ...
        if len(self.models) == 1:
            self.models[0].lr_decay(iteration, n_round)
            for i in self.models[0].optim.param_groups:
                print(i["lr"])

        max_nums, nums, agent_r_records, total_rewards, render_list = self.play(env=self.env, n_round=iteration,
                                                                                handles=self.handles,
                                                                                models=self.models, print_every=self.print_every,
                                                                                eps=variant_eps, render=self.render,
                                                                                train=self.train, cuda=self.cuda)
        if len(self.models) >= 2:
            ...
            if self.train:
                print('\n[INFO] main:{}'.format(info['main']))
                print('\n[INFO] opponent:{}'.format(info['opponent']))
                for k, v in info['main'].items():
                    self.writter.add_scalars(k, {k: v}, iteration + 1)
            ...
            info['main']['total_reward'] = total_rewards[0]
            info['main']['kill'] = max_nums[0] - nums[0]
            info['main']['ave_agent_reward'] = agent_r_records[0]
            if self.train:
                print('\n[INFO] main:{}'.format(info['main']))
                for k, v in info['main'].items():
                    self.writter.add_scalars(k, {k: v}, iteration + 1)
